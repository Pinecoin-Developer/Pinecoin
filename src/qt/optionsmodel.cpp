...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizepinecoinAmount"))
        settings.setValue("nAnonymizepinecoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizepinecoinAmount = settings.value("nAnonymizepinecoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizepinecoinAmount"))
        SoftSetArg("-anonymizepinecoinamount", settings.value("nAnonymizepinecoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizepinecoinAmount:
            nAnonymizepinecoinAmount = value.toInt();
            settings.setValue("nAnonymizepinecoinAmount", nAnonymizepinecoinAmount);
            emit anonymizepinecoinAmountChanged(nAnonymizepinecoinAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
